...
const COREABI = [
  ...
  const COREADDRESS = "0x593D3C25f143AcFA5881B1DA5345771CFA3b55a2";
      ...
      "outputs": [
        {
          ...
          "internalType": "struct NFTDEMUSICACOLLECTION.NFT[]",
          "name": "",
          "type": "tuple[]"
        }
      ],
  ...
  const COLLECTIONSADDRESS = "0x593D3C25f143AcFA5881B1DA5345771CFA3b55a2";
  const provider = new ethers.providers.JsonRpcProvider("https://speedy-nodes-nyc.moralis.io/10070b5bc12fd91e099c78ba/bsc/mainnet");
  const coreContract = new ethers.Contract(COREADDRESS, COREABI, provider);
  const collectionsContract = new ethers.Contract(COLLECTIONSADDRESS, COLLECTIONSABI, provider);

export default async function handler(req, res) {
    const address = JSON.stringify(req.query.address).replaceAll("\"", "");
    const Address = JSON.parse(address);
    try{
      const singlesAdr = await coreContract.getSinglesCollection(Address);
      console.log(singlesAdr.toString());
      const collection = new ethers.Contract(singlesAdr , COLLECTIONSABI , provider);
