...
function CollectionSectionMain(props) {
  const [selectedvalue, setSelectedvalue] = useState();
  const [selectedvalue2, setSelectedvalue2] = useState();
  const [isLoading, setIsLoading]  =  useState(true);
  const [noItems, setNoItems]  =  useState(true);
  const {currentTab, cid} = props
  ...
  const [ownedItems, setOwnedItems]  =  useState();
    ...
    useEffect(() => {
      const loadOwned  = async () => {
        console.log("OWNED", cid);
        setIsLoading(true);
        let items = await getSinglesByCollection(cid);
        if(items  ==  0){
          setNoItems(true);
        } else{
          setOwnedItems(items);
...
export default CollectionSectionMain