...
import { getSearchResults, getCollectedByAdress, getCollectionsByAdress } from '../../utils/helpers';
...
function Search() {
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      const loadSearch  = async () => {
        ...
        setIsLoading(true);
        let items = await getSearchResults(params);
        if(items  ==  0){
          setNoItems(true);
        } else{
          setOwnedItems(items);
  ...
  return (
    <div className={`sm:w-[90%] w-full min-h-[70vh] py-6 px-4 ml-auto relative`}>
        ...
        */}
        <div className='mb-5 font-semibold text-[12px]'>
            {ownedItems?.singlesByName.length} {language && language === "es" ? "NFTS por nombre" : "NFTS by name"}
        </div>
        {!noItems ? <div className='flex justify-start sm:gap-[2%] gap-[4%] flex-wrap'>
            {
              !isLoading && ownedItems.singlesByName && ownedItems.singlesByName.map((item, index) =>{
                return(
        ...
        <div className='mb-5 font-semibold text-[12px]'>
            {ownedItems?.nftCollections.length} {language && language === "es" ? "Colecciones" : "Collections"}
        </div>
        {!noItems ? <div className='flex justify-start sm:gap-[2%] gap-[4%] flex-wrap'>
          {
              !isLoading && ownedItems.nftCollections && ownedItems.nftCollections.map((item, index) =>{
                return(
                  <div key={index} className={`sm:w-[23%] w-[46%] mb-4`}>
                     <CollectionCard2 id={item.id} item={item} image={item.coverImage} text={item.name} adr={item.id}/>
                  </div>
                )
              })
          }
        ...
        <div className='mb-5 font-semibold text-[12px]'>
          {ownedItems?.users.length} {language && language === "es" ? "Artistas" : "Artists"}
        </div>
        {!noItems ? <div className='flex justify-start sm:gap-[2%] gap-[4%] flex-wrap'>
          {
              !isLoading && ownedItems.users && ownedItems.users.map((item, index) =>{
                return(
                  <div key={index} className={`sm:w-[23%] w-[46%] mb-4`}>
                     <CollectionCard id={item.id} item={item} image={item.media} text={item.name} adr={item.address}/>
                  </div>
                )
              })
          }
